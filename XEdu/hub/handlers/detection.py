...
class PalmHandDetectorHandler(DetectionHandler):
    """Multi-hand palm detector backed by the tested pure ONNX model."""
        ...
        for row in np.asarray(raw_output):
            ...
            boxes.append(
                [
                    ...
                    min(float(image_w), center_x + side / 2),
                    min(float(image_h), center_y + side / 2),
                ]
            )
            scores.append(score)
        return (
    ...
    def inference(
        ...
        **kwargs,
        ...
        boxes, scores = self.decode_palm_boxes(
            raw,
            image.shape,
